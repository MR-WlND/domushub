...
class ManualPaymentPageTest extends TestCase
{
    ...
    /**
     * Test AJAX search khi không có từ khóa trả về tất cả hóa đơn chưa thanh toán.
     */
    public function test_search_without_query_returns_all_invoices(): void
    {
        $admin = User::where('role', 'admin')->first();
        if (!$admin) {
            $this->markTestSkipped('Không có user admin trong DB.');
        ...
        $response->assertStatus(200)
                 ->assertJson(['success' => true, 'is_all' => true]);
    }
