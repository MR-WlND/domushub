...
<style>
    ...
    .modal-body { font-size: 13.5px; color: var(--text-secondary); margin-bottom: 16px; }
    .form-label-sm { font-size: 12px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: .04em; display: block; margin-bottom: 4px; }
...
<div class="invoice-detail-card">
    <div class="invoice-grid">
        ...
        <div class="info-group">
            ...
            <span class="info-value" style="color: var(--brand-600); font-size: 18px;">{{ number_format($invoice->total_amount) }} đ</span>
        </div>
        <div class="info-group">
    ...
    <div style="background: #f8fafc; border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 14px 18px; margin-bottom: 16px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-primary);">💰 Tiến trình thanh toán</span>
    ...
    <hr style="border: 0; border-top: 1px solid var(--border); margin: 24px 0;">
    <h3 style="font-size: 15px; font-weight: 700; margin-bottom: 12px; color: var(--green-600)">Lịch sử Thanh toán</h3>
    <table class="data-table">
        <thead>
            <tr>
                <th>Mã GD</th>
        ...
        <tbody>
            ...
            <tr style="{{ $pm->is_refunded ? 'opacity:.55;' : '' }}">
                ...
                <td class="amount" style="{{ $pm->is_refunded ? 'text-decoration:line-through;color:var(--text-muted)' : '' }}">
                ...
                <td>
                    <span class="recorder-badge">👤 {{ $pm->recorder->name }}</span>
                </td>
                ...
                <td>
                    @if($pm->is_refunded)
                        ...
                        @if($pm->refund_note)
                        <div style="font-size:11px;color:var(--text-muted);margin-top:3px;">{{ $pm->refund_note }}</div>
                        @endif
                    @else
        ...
        <form method="POST" action="{{ route('admin.invoices.mark-paid', $invoice) }}">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 14px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 5px;">
                    ...
                    <select name="payment_method" id="payment_method" class="period-select" style="width: 100%;">
