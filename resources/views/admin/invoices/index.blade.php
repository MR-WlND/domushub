...
<div class="inv-admin">
    ...
    <div class="inv-admin__filter-bar">
        <form method="GET" action="{{ route('admin.invoices.index') }}" class="inv-admin__filter-form">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm căn hộ, tòa..." class="inv-admin__input">

            <input type="month" name="month" value="{{ request('month') }}" class="inv-admin__input" style="flex:0;min-width:160px;" placeholder="Tháng/Năm">
                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex;gap:6px;flex-wrap:wrap;">
                            <a href="{{ route('admin.invoices.show', $invoice) }}" class="inv-admin__btn inv-admin__btn--sm inv-admin__btn--detail">Chi tiết</a>
                            <a href="{{ route('admin.invoices.print', $invoice) }}" target="_blank" class="inv-admin__btn inv-admin__btn--sm" style="background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;">🖨 In</a>
                            @if(!in_array($invoice->status, ['paid','cancelled']))
                                <form action="{{ route('admin.invoices.resend-notification', $invoice) }}" method="POST" style="display:inline;">
                                    @csrf
                                    <button type="submit" class="inv-admin__btn inv-admin__btn--sm" style="background:#eff6ff;color:#2563eb;border:1px solid #bfdbfe;" title="Gửi lại thông báo">🔔</button>
                                </form>
                                <button
                        ...
                        {{-- Hidden cancel form --}}
                        <form id="cancel-form-{{ $invoice->id }}" action="{{ route('admin.invoices.cancel', $invoice) }}" method="POST" style="display:none;">
                            @csrf
                        </form>
                    </td>
                </tr>
...
<style>
.inv-admin { max-width: 1200px; margin: 0 auto; padding: 24px 20px; }
...
.inv-admin__alert--error   { background: #fef2f2; border: 1px solid #fca5a5; color: #991b1b; }

/* Summary Cards */
.inv-summary-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; margin-bottom: 20px; }
.inv-summary-card { background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 16px 20px; }
.inv-summary-card--success { border-color: #bbf7d0; background: #f0fdf4; }
.inv-summary-card--danger  { border-color: #fca5a5; background: #fef2f2; }
.inv-summary-card--ok      { border-color: #bbf7d0; background: #f0fdf4; }
.inv-summary-card--warning { border-color: #fde68a; background: #fffbeb; }
.inv-summary-label { display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 6px; }
.inv-summary-val { display: block; font-size: 1.5rem; font-weight: 800; color: #0f172a; }
.inv-summary-card--success .inv-summary-val { color: #166534; }
.inv-summary-card--danger  .inv-summary-val { color: #991b1b; }
.inv-summary-card--warning .inv-summary-val { color: #92400e; }
...
.inv-admin__input  { flex: 1; min-width: 180px; padding: 7px 12px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 0.85rem; color: #1e293b; background: #f8fafc; }
.inv-admin__select { padding: 7px 12px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 0.85rem; color: #1e293b; background: #f8fafc; cursor: pointer; }

/* Buttons */
.inv-admin__btn { padding: 7px 14px; border-radius: 6px; font-size: 0.85rem; font-weight: 600; cursor: pointer; border: none; text-decoration: none; display: inline-flex; align-items: center; gap: 4px; }
...
.inv-admin__btn--sm { padding: 5px 10px; font-size: 0.78rem; }
.inv-admin__btn--detail { background: #eff6ff; color: #2563eb; border: 1px solid #bfdbfe; }
.inv-admin__btn--detail:hover { background: #dbeafe; }

/* Card / Table */
.inv-admin__card { background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; overflow: hidden; }
.inv-admin__card-header { display: flex; justify-content: space-between; align-items: center; padding: 12px 18px; border-bottom: 1px solid #e2e8f0; font-size: 0.85rem; font-weight: 600; color: #334155; }
.inv-admin__muted { color: #94a3b8; font-size: 0.8rem; }
.inv-admin__table { width: 100%; border-collapse: collapse; }
.inv-admin__table th { text-align: left; padding: 10px 14px; font-size: 0.75rem; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.04em; border-bottom: 1px solid #e2e8f0; background: #f8fafc; }
.inv-admin__table td { padding: 11px 14px; font-size: 0.85rem; color: #1e293b; border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
.inv-admin__table tr:hover td { background: #f8fafc; }
.inv-apt-code { font-family: monospace; font-size: 0.88rem; font-weight: 700; color: #0f172a; background: #f1f5f9; padding: 3px 8px; border-radius: 4px; }

/* Badges */
.inv-admin__badge { font-size: 0.72rem; font-weight: 600; padding: 3px 9px; border-radius: 4px; white-space: nowrap; }
.inv-admin__badge--paid      { background: #dcfce7; color: #15803d; }
.inv-admin__badge--unpaid    { background: #fef3c7; color: #b45309; }
.inv-admin__badge--partial   { background: #dbeafe; color: #1d4ed8; }
.inv-admin__badge--overdue   { background: #fee2e2; color: #b91c1c; }
.inv-admin__badge--cancelled { background: #e2e3e5; color: #383d41; }
...
.inv-admin__page-btn { padding: 6px 12px; border-radius: 6px; font-size: 0.8rem; font-weight: 600; border: 1px solid #e2e8f0; color: #475569; background: #fff; text-decoration: none; }
...
@media (max-width: 768px) {
    ...
    .inv-admin__filter-form { flex-direction: column; align-items: stretch; }
}
</style>
