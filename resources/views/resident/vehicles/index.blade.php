...
<div class="vp">
    ...
    @else
        ...
        <div class="vp-stats">
            ...
            <div class="vp-stat stat--approved">
                <div class="vp-stat__icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                </div>
                <div>
                    <span class="vp-stat__num color-approved">{{ $vehicles->where('status', 'approved')->count() }}</span>
                    <span class="vp-stat__label">Đã duyệt</span>
                </div>
            </div>

            <div class="vp-stat stat--rejected">
                <div class="vp-stat__icon">
                ...
                <div>
                    <span class="vp-stat__num color-rejected">{{ $vehicles->where('status', 'rejected')->count() }}</span>
                    <span class="vp-stat__label">Từ chối</span>
                </div>
            </div>
        </div>
                    ...
                    <div class="vp-card__right">
                        <div class="vp-badge-wrapper">
                            ...
                            @elseif($vehicle->status === 'approved')
                                <span class="vp-badge badge--approved">Đã duyệt</span>
                            @else
                                <span class="vp-badge badge--rejected">Từ chối</span>
                            @endif
                        </div>

                        @if($vehicle->status === 'approved' && $vehicle->qr_code)
                        ...
                        @if($vehicle->isPending())
                            <form method="POST"
                                  ...
                                  onsubmit="return confirm('Bạn có chắc chắn muốn hủy đăng ký xe này?')">
                                @csrf
                                @method('DELETE')
