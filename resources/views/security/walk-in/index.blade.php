...
/* OWNER CARD */
.wi-owner-card{background:#f0f7ff;border:1.5px solid #bfdbfe;border-radius:11px;padding:.8rem 1rem;display:flex;align-items:center;gap:.75rem;box-shadow:0 1px 4px rgba(0,0,0,.04)}
.wi-owner-avatar{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#2563eb,#1d4ed8);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:.85rem;flex-shrink:0}
...
.wi-btn-prev{padding:.85rem 1rem;background:#fff;color:#64748b;border:1.5px solid #e2e8f0;border-radius:12px;font-size:.9rem;font-weight:700;font-family:inherit;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:.5rem;transition:all .15s;flex:1}
.wi-btn-prev:hover{background:#f8fafc;border-color:#cbd5e1;color:#1e293b}
/* UTILS */
...
<div class="wi-sec">
    ...
    <div class="wi-sec-body">
        <div class="wi-grid2">
            ...
            <div class="wi-fld wi-span2">
                <label class="wi-lbl">Căn hộ đến thăm <em>*</em></label>
            ...
            {{-- Thông tin chủ hộ căn hộ --}}
            <div class="wi-fld wi-span2" id="owner-box" style="display:none">
                <div id="owner-info" class="wi-owner-card">
                    <div class="wi-owner-avatar" id="owner-avatar">?</div>
                    <div class="wi-owner-details">
                        <div class="wi-owner-name" id="owner-name">—</div>
                        <div class="wi-owner-phone" id="owner-phone">—</div>
                    </div>
                </div>
            </div>
    ...
    {{-- Step navigation: Part 1 → Part 2 --}}
    <div class="wi-sec-body">
        <div class="wi-step-nav">
            <div></div>
            <button type="button" class="wi-btn-main" onclick="goToStep(2)" style="max-width:260px;margin-left:auto">
                Tiếp tục → Chụp ảnh
...
<div class="wi-sec">
    ...
    {{-- Step navigation: Part 2 → Part 3 / ← Part 1 --}}
    <div class="wi-sec-body">
        <div class="wi-step-nav">
            <button type="button" class="wi-btn-prev" onclick="goToStep(1)">
            ...
            <button type="button" class="wi-btn-main" onclick="goToStep(3)" style="max-width:260px">
                Xem xác nhận
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
            </button>
        </div>
...
<div class="wi-sec">
    ...
    <div class="wi-sec-body">
        ...
        {{-- Submit button inside Part 3 --}}
        <button type="button" id="btn-submit" class="wi-btn-main" onclick="doSubmit()" style="margin-top:1rem">
        ...
        {{-- Step navigation: Part 3 → submit / ← Part 2 --}}
        <div class="wi-step-nav">
            <button type="button" class="wi-btn-prev" onclick="goToStep(2)">
                <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
                Quay lại chụp ảnh
            </button>
        </div>
        <div class="wi-footer-links" style="margin-top:.85rem">
...
const APT_OWNERS = {
    @foreach($apartmentOwners as $aptId => $owner)
    {{ $aptId }}: {
        @if($owner)
        id:{{ $owner['id'] }},
...
function goToStep(n){
    if(n===2){
        // Validate Part 1 before going to Part 2
        const name=document.getElementById('guest_name').value.trim();
        const aptId=document.getElementById('apartment_id').value;
        const ownerId=document.getElementById('confirmed_by_resident').value;
        ...
        if(!ownerId){showToast('Căn hộ chưa có chủ hộ để gửi thông báo.','error');return;}
    }
    setStep(n);
    // Scroll to the target section
    const secs=document.querySelectorAll('.wi-sec');
    if(secs[n-1]) secs[n-1].scrollIntoView({behavior:'smooth',block:'start'});
}
...
function pickApt(id,label){
    ...
    // Hiển thị thông tin chủ hộ và tự động set
    const owner=APT_OWNERS[id];
    if(owner && owner.id){
        showApartmentOwner(id);
        document.getElementById('confirmed_by_resident').value=owner.id;
    }else{
...
function showApartmentOwner(aptId){
    const box=document.getElementById('owner-box');
    const owner=APT_OWNERS[aptId];
    if(owner && owner.id){
        document.getElementById('owner-avatar').textContent=(owner.name||'?').charAt(0).toUpperCase();
        document.getElementById('owner-name').textContent='Chủ hộ: '+owner.name;
        document.getElementById('owner-phone').textContent=owner.phone||'Không có số điện thoại';
        box.style.display='block';
    }else{
        document.getElementById('owner-name').textContent='Căn hộ chưa có chủ hộ';
...
function updateReview(){
    ...
    const aptId=document.getElementById('apartment_id').value;
    const ownerId=document.getElementById('confirmed_by_resident').value;
    const owner=aptId?APT_OWNERS[parseInt(aptId)]:null;
    const ownerName=owner&&owner.id?owner.name:'';
    const note=document.getElementById('note').value.trim();
    document.getElementById('rv-name').textContent=name||'—';
    document.getElementById('rv-phone').textContent=phone||'—';
    document.getElementById('rv-apt').textContent=aptLbl?(aptLbl+(ownerName?' ('+ownerName+')':'')):'—';
    ...
    else if(!ownerId) notice.textContent='Căn hộ chưa có chủ hộ, không thể gửi thông báo.';
    else notice.innerHTML='Sẵn sàng gửi thông báo đến cư dân <strong>'+ownerName+'</strong> và ghi nhận khách vào.';
}
['guest_name','guest_phone','note'].forEach(id=>{
...
async function doSubmit(){
    ...
    const aptId=document.getElementById('apartment_id').value;
    const ownerId=document.getElementById('confirmed_by_resident').value;
    const aptData=APT_DATA.find(a=>a.id===parseInt(aptId));
    const owner=aptId?APT_OWNERS[parseInt(aptId)]:null;
    const ownerName=owner?.name||'';
    if(!name){showToast('Vui lòng nhập họ tên khách.','error');document.getElementById('guest_name').focus();return;}
    if(!aptId){showToast('Vui lòng chọn căn hộ.','error');document.getElementById('apt_filter').focus();return;}
    if(!ownerId){showToast('Căn hộ chưa có chủ hộ, không thể gửi thông báo.','error');return;}
    ...
    try{
        const r=await fetch(URL_STORE,{
            ...
            body:JSON.stringify({
                ...
                resident_to_meet:ownerName,
                confirmed_by_resident:ownerId,
        ...
        if(data.success){showToast(data.message+' · Đã thông báo chủ hộ.','success');setTimeout(resetForm,1400);}
